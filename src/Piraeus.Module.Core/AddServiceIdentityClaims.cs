...
namespace Piraeus.Module
{
    ...
    {
        [Parameter(HelpMessage = "Semi-colon delimited list of claim types.  Must match number of claim values.",
            Mandatory = true)]
        public string ClaimTypes;

        [Parameter(HelpMessage = "Semi-colon delimited list of claim values.  Must match number of claim types.",
            Mandatory = true)]
        ...
        protected override void ProcessRecord()
        {
            ...
            string url =
                string.Format(
                    $"{ServiceUrl}/api/serviceidentity/addclaimtypes?key={Name}&claimtypes={ClaimTypes}&claimvalues={ClaimValues}");
